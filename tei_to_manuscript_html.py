...
# Adds support for <caesura>, <gap>, <sic>, <space> and hover tooltips for <note place="...">
...
class Page:
    ...
    def add_foot(self, txt):
        num = self.fn_counter; self.fn_counter += 1
        fid = f"fn{num}"
...
# ---------- Render helpers ----------
def render_lb(el):
    n = el.attrib.get("n", ""); br = el.attrib.get("break", "yes")
    if br == "no": return f'-<br/><span class="linenum">{escape(n)}</span>'
    return f'<br/><span class="linenum">{escape(n)}</span>'

def render_add(el):
    place = el.attrib.get("place", ""); tooltip = place if place else "supplied"
...
def render_unclear(el, page=None):
    out = ""; 
    if el.text: out += escape(el.text)
    for c in el:
        out += render_element(c, page)
        if c.tail: out += escape(c.tail)
...
def render_surplus(el):
    reason = el.attrib.get("reason", ""); tooltip = f' title="{escape(reason)}"' if reason else ""
    return f'<span class="surplus"{tooltip}>{"".join(escape(t) for t in el.itertext())}</span>'

def render_choice(el):
    orig = el.find(".//{*}orig"); corr = el.find(".//{*}corr")
...
def render_quote(el):
    src = el.attrib.get("source", ""); tooltip = ""
    if src:
        parts = src.split(); items = []
        for part in parts:
            if part.startswith("#"):
                key = part[1:]
                if key in BIBLIO: items.append(BIBLIO[key])
            else: items.append(part)
        tooltip = "; ".join(items)
    ...
    for c in el:
        tag = localname(c.tag)
        out.append(render_lb(c) if tag == "lb" else render_element(c))
        if c.tail: out.append(escape(c.tail))
    return f'<span class="quote" {title}>{"".join(out)}</span>'

# ---------- New tags ----------
def render_caesura(el): return '<span class="caesura">‖</span>'
def render_gap(el):
    reason = el.attrib.get("reason", "gap"); unit = el.attrib.get("unit", ""); qty = el.attrib.get("quantity", "")
    desc = f"{reason}"
    if qty and unit: desc += f" ({qty} {unit})"
    elif qty: desc += f" ({qty})"
    return f'<span class="gap" title="{escape(desc)}">[...]</span>'
def render_sic(el): return f'<span class="sic" title="sic">{escape("".join(el.itertext()))}</span>'
def render_space(el):
    unit = el.attrib.get("unit", ""); qty = el.attrib.get("quantity", "")
    tooltip = f"{qty} {unit}".strip() or "space"
    width = max(1, int(qty)) if qty.isdigit() else 1
    return f'<span class="space" title="{escape(tooltip)}">{"&nbsp;"*width}</span>'

# ---------- Recursive renderer ----------
def render_element(el, page=None):
    tag = localname(el.tag); out = ""
    ...
    if tag == "subst":
        del_el = el.find(".//{*}del"); add_el = el.find(".//{*}add"); parts = []
        if del_el is not None: parts.append(render_del(del_el))
        if add_el is not None: parts.append(render_add(add_el))
        return "".join(parts)

    elif tag == "note":
        nid = el.attrib.get("id", ""); resp = el.attrib.get("resp", ""); place = el.attrib.get("place", "")
        if nid in SKIP_NOTE_IDS: return ""
        if resp == "editorial" and page:
            return page.add_foot(''.join(el.itertext()).strip())

        place_map = {
            "right": "brought from right margin",
            "left": "brought from left margin",
            "top": "brought from top margin",
            "bottom": "brought from bottom margin"
        }
        tooltip = place_map.get(place, "note")
        inner = ""
        if el.text: inner += escape(el.text)
        for c in el:
            inner += render_element(c, page)
            if c.tail: inner += escape(c.tail)
        return f'<span class="note" data-place="{escape(place)}" title="{tooltip}">{inner}</span>'
        ...
        head = el.find("./{*}head"); 
        if head is not None: out += f"<h2>{escape(''.join(head.itertext()))}</h2>"
        for c in el:
...
def tei_to_html(infile, outfile):
    ...
    for bibl in root.findall(".//tei:listBibl//tei:bibl", ns):
        ...
        BIBLIO[bid] = f"{title} – {author}" if title and author else (title or author)
    body = root.find('.//tei:text/tei:body', ns)
    if body is None:
        print("No <body>"); return
    pages, current = [], Page("start"); pages.append(current)
    for child in list(body):
        tag = localname(child.tag)
        if tag == "pb":
            current = Page(child.attrib.get("n", "?")); pages.append(current); continue
    ...
    html = [
        ...
        .add { background:cyan; }
        .del { background:#F5F5F5; color:gray; text-decoration:line-through gray 1.5px; display:inline-block; }
        ...
        .verse * { color:inherit; }
        .surplus { background:tan !important; text-decoration:line-through 2px gray; color:#2a1e0e; }
    ...
    for i, pg in enumerate(pages):
        if i == 0 and not any(s.strip() for s in pg.html_parts): continue
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Convert TEI XML → manuscript HTML")
    parser.add_argument("input"); parser.add_argument("output")
    args = parser.parse_args(); tei_to_html(args.input, args.output)
